...
namespace PuppySharpPdf.Standard.Renderers
{
    public class PuppyPdfRenderer
    {
        ...
        public async Task<Result<byte[]>> GeneratePdfFromUrlAsync(string url, Action<Standard.Renderers.Configurations.PdfOptions> pdfOptions)
        {
            if (url is null)
            {
                return Result.Invalid(new List<ValidationError> { new ValidationError { ErrorMessage = "Url can't be empty", ErrorCode = "400" } });
            ...
            var customPdfOptions = new Standard.Renderers.Configurations.PdfOptions();
        ...
        public async Task<Result<byte[]>> GeneratePdfFromUrlAsync(string url, Standard.Renderers.Configurations.PdfOptions pdfOptions)
        {
            if (url is null)
            {
                return Result.Invalid(new List<ValidationError> { new ValidationError { ErrorMessage = "Url can't be empty", ErrorCode = "400" } });
        ...
        public async Task<Result<byte[]>> GeneratePdfFromHtmlAsync(string html)
        {
            ...
            try
            {
                ...
                await page.SetJavaScriptEnabledAsync(true);
                await page.WaitForNetworkIdleAsync();
                var result = await page.PdfDataAsync(new Standard.Renderers.Configurations.PdfOptions().MappedPdfOptions);
                return Result.Success(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<Result<byte[]>> GeneratePdfFromHtmlAsync(string html, Action<Standard.Renderers.Configurations.PdfOptions> options)
        {
            if (html is null)
            {
                return Result.Invalid(new List<ValidationError> { new ValidationError { ErrorMessage = "Html string can't be empty", ErrorCode = "400" } });
            ...
            var pdfOptions = new Standard.Renderers.Configurations.PdfOptions();
        ...
        public async Task<Result<byte[]>> GeneratePdfFromHtmlAsync(string html, Standard.Renderers.Configurations.PdfOptions pdfOptions)
        {
            if (html is null)
            {
                return Result.Invalid(new List<ValidationError> { new ValidationError { ErrorMessage = "Html string can't be empty", ErrorCode = "400" } });
